...
from data.audioVisual_dataset import generate_spectrogram_complex, get_preprocessing_pipelines, load_frame
...
def main():
	...
	# load data
	mouthroi_1 = h5py.File(opt.mouthroi1_path, "r")["data"][...]
	mouthroi_2 = h5py.File(opt.mouthroi2_path, "r")["data"][...]
	...
	while sliding_window_start + samples_per_window < audio_length:
		...
		data['audio_spec_mix'] = torch.FloatTensor(audio_mix_spec).unsqueeze(0)
		data['mouthroi1'] = torch.FloatTensor(segment1_mouthroi).unsqueeze(0).unsqueeze(0)
		data['mouthroi2'] = torch.FloatTensor(segment2_mouthroi).unsqueeze(0).unsqueeze(0)
		data['audio_spec1'] = torch.FloatTensor(audio_spec_1).unsqueeze(0)
		data['audio_spec2'] = torch.FloatTensor(audio_spec_2).unsqueeze(0)
		data['frame1'] = frames_1
		data['frame2'] = frames_2
		#don't care below
	...
	if len(mouthroi_1) - frame_index_start < opt.num_frames or len(mouthroi_2) - frame_index_start < opt.num_frames:
		frame_index_start -= opt.num_frames - min(len(mouthroi_1) - frame_index_start, len(mouthroi_2) - frame_index_start)
	segment1_mouthroi = mouthroi_1[frame_index_start:(frame_index_start + opt.num_frames), :, :]
	segment2_mouthroi = mouthroi_2[frame_index_start:(frame_index_start + opt.num_frames), :, :]
	#transform mouthrois
	segment1_mouthroi = lipreading_preprocessing_func(segment1_mouthroi)
